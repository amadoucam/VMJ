...
class Invoice {
	...
	protected function tax(&$plus) {
		...
		$this->image->text($this->formatNumber($this->getTotalPrice(FALSE)) * 0.8 . ' ' . $this->order->getPayment()->getCurrency(), 2260, $plus + 65, function (Font $font) {
			$font->align('center');
			$font->color($this->template->getFontColor());
			$font->file($this->template->getFont());
			$font->size(27);
		...
		$this->image->text($this->order->getPayment()->getTax() . ' €', 2260, $plus + 65, function (Font $font) {
			$font->align('center');
			$font->color($this->template->getFontColor());
			$font->file($this->template->getFont());
			$font->size(27);
